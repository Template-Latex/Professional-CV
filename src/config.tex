...
\def\fontdocument {default}       % Tipografía base, ver manual
...
\def\headeritemsmargin {0.5}      % Margen vertical ítems del header [cm]
...
\def\hfstyle {style10}            % Estilos encabezado y pie de pag (11 disp.)
\def\indentwidth {15}             % Tamaño indentación en distintos obj. [pt]
\def\institutionbotmargin {0.23}  % Margen inferior, factor de \indentwidth
...
\def\institutioninnermargin {1.0} % Margen exterior, factor de \indentwidth
\def\institutionoutermargin {0.5} % Margen exterior, factor de \indentwidth
\def\institutiontitlebold {false} % Título institución en negrita
\def\personaltabledatapic {true}  % Muestra/Oculta la foto tabla antecedentes
\def\showuserphotoborder {true}   % Muestra un recuadro en la foto de perfil
\def\summaryfontsize {\small}     % Tamaño de la fuente de texto en summary
\def\userphotosize {3.5}          % Dimensiones de la foto del usuario [cm]
\def\usersignaturesize {5.0}      % Ancho de la foto de la firma [cm]
\def\writebirthdayheader {false}  % Escribe año nacimiento encabezado
\def\writelastchangeheader{false} % Escribe fecha último cambio en encabezado
...
\def\cvblocklinecolor {gray!35}   % Color de las líneas divisor. de los bloques
\def\datecolor {gray!90}          % Color fechas en entradas de instituciones
...
\def\instentrytitlecolor {black}  % Color del título en instituciones
\def\instnamecolor {dark-blue}    % Color nombre de las instituciones (+url)
...
\def\urlcolor {blue}              % Color enlaces web en cuerpo del documento
...
\def\cfgpdfcenterwindow {true}    % Centra la ventana al abrir el pdf
...
\def\namelastchange{Último cambio el} % Nombre apartado último cambio
\def\namepersonaldata{Antecedentes Personales} % Título antecedentes personales
\def\namesummary {}                   % Título sección summary. Vacío lo oculta
